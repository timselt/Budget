...
## ADR-0016 — Mutabakat Modülü Entity ID Tipi: int (Guid değil)

**Tarih:** 2026-04-19
**Statü:** Kabul edildi
**Karar Sahibi:** Timur Selçuk Turan
**Bağlı Spec:** `docs/Mutabakat_Modulu/docs/specs/01_phase1_domain_model.md` §3, `00b_prereq_pricebook.md` §2
**Bağlı Design:** `docs/plans/2026-04-19-reconciliation-sprint2-3-design.md` §D

### 1. Bağlam

Mutabakat modülü spec'leri (Faz 1 domain model + 00a/00b/00c önkoşulları) entity PK'ları için **UUID** tipinde alanlar tanımlar. Sprint 1 ve önkoşul implementation'ı ise mevcut BudgetTracker projesinin yerleşik pattern'iyle **int** (identity) PK kullanmıştır. Explore ajan raporu bu sapmayı tespit etti (00b kontrol noktası 1-2 ⚠️).

Karar gerekçesi Sprint 2 başında netleştirilmeli: ya tüm mutabakat entity'leri Guid'e geçirilir (büyük refactor, migration zinciri + FK ilişkileri + DTO/API/test/seed güncellemesi), ya mevcut int korunup kayda alınır.

Projenin mevcut 16+ entity'sinin tamamı int identity kullanıyor (`Customer`, `Product`, `Contract`, `PriceBook`, `BudgetVersion`, vb.). Mutabakat entity'leri de aynı pattern'le inşa edildi.

### 2. Karar

**Mutabakat modülü entity ID tipi `int` (identity) olarak korunur.** Spec'teki UUID örnekleri referans amaçlıdır; implementation pragmatik olarak mevcut projeyle tutarlı int'i kullanır.

Etkilenen entity'ler (Sprint 1 ve Sprint 2):
- `ReconciliationBatch`, `ReconciliationSourceRow`, `ReconciliationCase`, `ReconciliationLine`, `ReconciliationDecision`, `AccountingInstruction`, `RiskRuleSet`
- `Contract` (ADR-0015'te zaten int), `PriceBook`, `PriceBookItem`

### 3. Reddedilen Alternatifler

- **Tüm mutabakat entity'lerini Guid PK'ya geçirmek.** Red: 7 tablo × migration + 30+ controller action + 50+ test + 5 UI sayfası güncellemesi → tahminen 3-5 iş günü ek maliyet, Sprint 2 planını bozar. Dış tarafa UUID göstermiyoruz (muhasebe export dosyasında `instruction_id` string olarak UUID tutabilir ama entity PK'sına bağlı değil).
- **Karışık model** (yeni entity'ler Guid, eskiler int). Red: FK tipi uyumsuzluğu + DbContext karmaşası + dev deneyimi bozulur.
- **Tek bir entity'de Guid (AccountingInstruction), diğerlerinde int.** Red: Sprint 2'de AccountingInstruction hâlâ iskelet, Faz 2'ye kadar kullanılmayacak; karar Faz 2'ye ertelenebilir ama şimdi ayrıca dokümante ederek gelecekteki karar yükünü azaltıyoruz.

### 4. Sonuçlar

**Olumlu:**
- Sprint 2 hedef tarihi korunur, ek refactor yükü yok.
- Mevcut DbContext pattern'i, EF Core identity config, seed script'leri değişmez.
- Integration test fixture'ları (Testcontainers + seed) int ID beklentisiyle çalışır.

**Olumsuz / Takip gereken:**
- Spec dokümanlarındaki UUID örnekleri (özellikle `01_phase1_domain_model.md` §3.2-3.7) kod ile birebir uyumsuz — dokümantasyonda "implementation int kullanır; ADR-0016" dipnotu Sprint 2 içinde eklenecek.
- Dış API tüketicilerine (muhasebe ERP entegrasyonu Faz 2) `instruction_id` için iki katman gerekebilir: iç int PK + dış string reference (opsiyonel UUID). Karar Faz 2'ye ertelendi.
- Eğer ilerde sistem federe (multi-region, merge-able) hale gelirse int collision riski doğar. Bu MVP kapsam dışı.

### 5. Kabul Kriterleri

- [x] Karar dokümante edildi (bu ADR)
- [ ] `01_phase1_domain_model.md` §3'e dipnot ("Implementation int PK; rationale: ADR-0016") — Sprint 2 Task 13 (dokümantasyon kapanış)
- [ ] Sprint 2 kapanışında spec uyumsuzluğu CHANGELOG'da not düşülür
